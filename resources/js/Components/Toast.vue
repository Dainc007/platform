...
const toastClasses = computed(() => {
    return `flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 transition-opacity duration-1000 ${visible.value ? '' : 'opacity-0'}`;
});
...
const iconClasses = computed(() => {
    ...
    switch (props.type) {
        ...
        case 'info':
            return `${baseClasses} text-blue-500 bg-blue-100 dark:bg-blue-800 dark:text-blue-200`;
        default:
            return `${baseClasses} text-green-500 bg-green-100 dark:bg-green-800 dark:text-green-200`;
    }
});
...
function markAsRead(id) {
    Inertia.patch(route('notifications.update', id), {}, {
        preserveState: true,
        preserveScroll: true,
    });
}
...
<template>
        <div :id="`toast-${type}-${id}`" :class="toastClasses" role="alert">
            ...
            <button @click="markAsRead(id)" type="button" class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" :data-dismiss-target="`#toast-${type}-${id}`" aria-label="Close">
