            ...
            <div v-if="$page.props.auth.isAdmin" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col items-start space-y-4">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        <tr v-for="vacation in upcomingVacations" v-if="vacations" :key="vacation.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="col"
                                class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                <div class="ps-3">
                                    <div class="text-base font-semibold">{{vacation.user.name}} <span class="text-yellow-500">({{ vacation.start_at }} - {{ vacation.end_at }})</span>
                                         </div>
                                    <div :class="getStatusClass(vacation.status)">{{ $t('vacation.status.' + vacation.status)}}</div>
